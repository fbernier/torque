...
// Cap recursive DOM and visitor parsing before stack exhaustion.
    ...
    R: Reader<'de>,
{
    ...
    fn parse_number_inner(
        &mut self,
        first: u8,
    ) -> std::result::Result<ParserNumber, sonic_number::Error> {
    ...
    ///
    /// `key` routes object keys through key-specific visitor hooks. Their
    /// defaults preserve ordinary string visitor behavior.
    #[inline(always)]
        ...
        V: JsonVisitor<'de>,
    {
        if self.cfg.use_rawnumber {
            ...
            // Preserve the token so overflowed integer literals reach the visitor exactly.
            let start = self.read.index() - 1;
            let ok = match self.parse_number_inner(first) {
                Ok(ParserNumber::Float(f)) => {
                    let slice = self.read.slice_unchecked(start, self.read.index());
                ...
                Ok(ParserNumber::Unsigned(f)) => vis.visit_u64(f),
                Ok(ParserNumber::Signed(f)) => vis.visit_i64(f),
                Err(sonic_number::Error::FloatMustBeFinite) => {
                    return self.parse_overflow_int(first, start, vis);
                }
                Err(err) => return Err(self.error(err.into())),
            };
            check_visit!(self, ok)
        }
    }

    /// Only a finite-float failure needs a second token scan. Ordinary numbers
    /// retain sonic-number's fast path, and DOM visitors still reject infinities.
    #[cold]
    #[inline(never)]
    fn parse_overflow_int<V>(&mut self, first: u8, start: usize, vis: &mut V) -> Result<()>
    where
        V: JsonVisitor<'de>,
    {
        let failed_at = self.read.index();
        self.read.set_index(start + 1);
        let raw = self.skip_number(first)?;
        if is_integer_token(raw.as_bytes()) {
            // `skip_number` checks the number grammar but leaves its terminator
            // to the container/document parser. Reject junk before converting a
            // potentially huge integer; a legal delimiter is still checked there.
            if self
                .read
                .peek()
                .is_some_and(|b| !matches!(b, b',' | b']' | b'}') && !is_whitespace(b))
            {
                return perr!(self, InvalidNumber);
            }
            let fallback = if first == b'-' {
                f64::NEG_INFINITY
            } else {
                f64::INFINITY
            };
            if vis.visit_overflow_int(raw, fallback) {
                return Ok(());
            }
        }
        // A default visitor cannot represent this token. Preserve the original
        // float error and its position instead of allowing a nonfinite Value.
        self.read.set_index(failed_at);
        perr!(self, FloatMustBeFinite)
    }
    ...
    ///
    /// `depth` carries the shared nesting budget into recursive containers.
    #[inline(always)]
    fn dispatch_value<V>(
        &mut self,
        ch: Option<u8>,
