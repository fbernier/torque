...
impl ExtractPlan {
    ...
    /// Canonical result slot for every requested path, in original order.
    /// Duplicate paths share their first slot. Paths beyond the nesting limit
    /// retain their own missing slot. Identities remain valid after `finish`.
    pub fn result_slots(&self) -> &[usize] {
        &self.result_slots
    }

    /// Whether any requested path shares a terminal with an earlier result.
...
/// Extracts each plan terminal once, without cloning duplicate selections.
///
/// The vector retains the original slot count. Only canonical slots from
/// [`ExtractPlan::result_slots`] are populated; duplicate slots remain `None`.
/// Consumers can materialize a canonical value once and reuse its result in
/// every aliased slot. [`extract`] retains the expanded, owned-value API.
pub fn extract_unique<'de, Input: JsonInput<'de>>(
    json: Input,
    plan: &ExtractPlan,
    validate: Validate,
    keys: Keys,
