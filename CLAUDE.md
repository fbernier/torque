...
Lookups dispatch on **what the caller brings**, since the document's size cannot report it. Path count first: they build one result per path, so a `pointers()` handle carrying 100k paths is 100k terms of work against a two-byte document, and 100k pointers on a normal scheduler measured 11.5 ms of wakeup latency for every other process on it. `compile_pointers/2` returns `{resource, path_count, path_bytes}` so both are known without walking the caller's list — `length/1` on a large list is the cost being avoided — and `get_many/2`, `get_many_nil/2`, `get_many_defaults/2` and `parse_get_many_nil/2` go dirty at `@dirty_path_count` (2048) or more. A raw path list has no stored count, so `many_paths?/1` walks at most that many cells, summing path bytes as it goes and stopping at whichever answer comes first — because a path is itself work: it is scanned to split it and again to unescape a `~` segment, and one 32 MiB path took 3.8 ms against `{}`, 23 ms with escapes, reported as four reductions. The handle carries the byte total for the same reason, and sums the *pointer* bytes rather than compiled segment bytes so the two forms of one path set answer identically: compiling removes the per-call split and unescape, not the key bytes each lookup still compares, and a one-path handle holding a 1 MB key measured 26 µs a lookup against the raw list's 135 µs. `Torque.dirty_paths?/1` is that decision for both forms, exposed for tests, and their agreement is pinned in `test/pointer_test.exs`. `get/2,3` and `length/2` dispatch on `byte_size(path)` for the same reason, and `get/3` answers through `get/2` so both arities route alike. `compile_paths` takes the same walk: it is startup work, but a three-path set compiles in 1.3 µs and a dirty hop costs about another one (measured 1.33 -> 2.29 µs), while 2048 paths take 400 µs and a 100k-path set ~50 ms. Same threshold, same predicate. `get_many_defaults/2` asks the same question of a map without building a key list: `map_size/1` answers the count outright, and `:maps.iterator/1` sums path bytes forward, stopping at the threshold. The threshold is calibrated on `get_many_defaults/2`, the slowest per path (p99 258 µs at 2048, 667 µs at 4096, against a target of half a millisecond); below it the dispatch is unmeasurable, and at it dirty costs 0.5-5% throughput.
...
What a call reports is accumulated in the units the work is done in and converted once, at the end, by `Work::nodes` (`decoder.rs`): comparisons, indexed members, and key or pointer bytes, alongside the exact `nodes` count that `value_to_term` and each emitted result contribute. Converting at each site that produced work discarded its remainder, and lookups are mostly remainder — scans of 31 members rounded to nothing however many times a batch made them, so 2047 of them charged only the results they emitted. `Work` is threaded through `pointer_lookup`, `object_get` and the memo; `value_to_term` keeps its exact `&mut usize` because it has nothing to round.
